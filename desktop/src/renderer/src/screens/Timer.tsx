...
export function TimerScreen({ userId, deviceId, onOpenProfile, onOpenHealth, onOpenPairing, onOpenQRGenerator, onOpenProject, onCreateProject }: TimerScreenProps): JSX.Element {
  ...
  // Sync customMins with user settings
  useEffect(() => {
    if (user?.workDurationMins) {
      setCustomMins(user.workDurationMins)
    }
  }, [user?.workDurationMins])
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval)
  }, [timerState, deviceId, lockDevice])
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0a0a0b]">
            ...
            <>
              ...
              {onOpenProfile && (
                <button
                  ...
                  className="p-2.5 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900 rounded-xl transition-all"
                  title="Profile"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className={`p-2.5 rounded-xl transition-all ${showSettings ? 'text-zinc-200 bg-zinc-800' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900'}`}
                title="Settings"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Settings Panel */}
      {showSettings && isIdle && (
        <div className="absolute top-16 right-4 w-72 glass-card p-5 z-30 fade-in-up">
          <h3 className="text-sm font-semibold text-zinc-200 mb-4">Session Duration</h3>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-mono text-zinc-300">{customMins} min</span>
            </div>
            <input
              type="range"
              min="5"
